...
model = handDenseNet(growth_rate = GROWTH_RATE, lastLinear = 60)
# model = Conv2Dense2()
...
if __name__ == '__main__':
    ...
    weights_file = './models/denseBigger.h5'
    json_file_name = "modelDenseBigger.json"

    # save_model_keras(model, dir_json = json_file_name, dir_name_weight = weights_file)
    # model = firstTrain(input_shape, data_train, all_test, epochs = EPOCHS, dir_name_weight = weights_file, dir_json = json_file_name, model = model)
